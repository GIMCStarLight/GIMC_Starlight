...
function extractBilibiliId(explicitId: string | undefined, homeLink: string | undefined): string | undefined {
...
function main() {
  ...
  const normalizedRows: Record<string, unknown>[] = rows.map((r) => {
    ...
    let account_id = extractBilibiliId(explicitId, home_link);
    if (!account_id) account_id = explicitId || `bili:${(account_name || '').slice(0, 16)}`;
    ...
    // B站的报价字段：将“植入视频平台价”映射为“21s-60s报价”，将“定制视频平台价”映射为“60s+报价”
    const price_21_60 = pick(r, ['植入视频平台价']);
    const price_60_plus = pick(r, ['定制视频平台价']);

    const final_link = home_link || (account_id ? `https://space.bilibili.com/${account_id}` : '');
    return {
      账号平台: platform,
      账号名称: account_name,
      账号ID: account_id || '',
  ...
  const outFile = path.join(outDir, 'B站.clean.csv');
