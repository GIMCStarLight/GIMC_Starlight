...
function extractWeiboId(explicitId: string | undefined, homeLink: string | undefined): string | undefined {
...
function main() {
  ...
  const normalizedRows: Record<string, unknown>[] = rows.map((r) => {
    ...
    let account_id = extractWeiboId(explicitId, home_link);
    if (!account_id) account_id = explicitId || `weibo:${(account_name || '').slice(0, 16)}`;
    ...
    // 微博通常报价不稳定，这里不强制映射；若源文件存在同名价位列则保留
    const price_21_60 = pick(r, ['21s-60s报价', '21-60s视频预估报价']);
    const price_60_plus = pick(r, ['60s+报价', '60s以上视频预估报价']);

    const final_link = home_link || (account_id ? `https://weibo.com/u/${account_id}` : '');
    return {
      账号平台: platform,
      账号名称: account_name,
      账号ID: account_id || '',
  ...
  const outFile = path.join(outDir, '微博.clean.csv');
