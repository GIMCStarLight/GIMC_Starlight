...
function extractAccountId(explicitId: string | undefined, homeLink: string | undefined): string | undefined {
...
function main() {
  ...
  const normalizedRows: Record<string, unknown>[] = rows.map((r) => {
    ...
    let account_id = extractAccountId(explicitId, home_link);
    if (!account_id) {
      // 若仍缺失，则尝试用“用户id”或基于名称生成
      account_id = explicitId || `kuaishou:${(account_name || '').slice(0, 16)}`;
    }
    ...
    // 若有ID则构造主页链接
    const final_link = home_link || (account_id ? `https://www.kuaishou.com/profile/${account_id}` : '');
  ...
  const outFile = path.join(outDir, '快手.clean.csv');
