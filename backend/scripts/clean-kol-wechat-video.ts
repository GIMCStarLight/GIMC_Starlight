...
function main() {
  ...
  const normalizedRows: Record<string, unknown>[] = rows.map((r) => {
    const platform = '视频号';
    const account_name = (pick(r, ['账号名称', '账号名', '昵称']) || '')
      .replace(/[“”]/g, '"')
      .trim();
    // 视频号一般无稳定主页链接
    ...
    // 标头可能包含换行，如“账号类型\n（如美妆、母婴、汽车等）”、“粉丝量\n（w）”，做兼容并统一为标准列名
    const fansRaw = pick(r, ['粉丝量（w）', '粉丝量\n（w）', '粉丝量(W)', '粉丝量w', '粉丝量']);
    const category = pick(r, ['账号类型', '账号类型\n（如美妆、母婴、汽车等）', '类型', '分类']);

    // 视频号的报价字段：互选（1/60s）参考价 & 互选（60s+）原创参考价
    const price_21_60 = pick(r, ['互选（1/60s）参考价', '1/60s参考价']);
    const price_60_plus = pick(r, ['互选（60s+）原创参考价', '60s+原创参考价']);
    ...
    return {
      ...
      主页链接: home_link,
      '粉丝量（w）': fansRaw || '',
      '互选（1/60s）参考价': price_21_60 || '',
      '互选（60s+）原创参考价': price_60_plus || '',
      所属机构名: org_name || '',
      账号类型: category || '',
      达人属性: pick(r, ['达人属性', '标签']) || '',
  ...
  const outFile = path.join(outDir, '视频号.clean.csv');
