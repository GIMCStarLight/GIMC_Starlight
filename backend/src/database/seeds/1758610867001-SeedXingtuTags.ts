...
export default class SeedXingtuTags1758610867001 {
  public async run(dataSource: DataSource): Promise<any> {
    const tagRepository = dataSource.getRepository(Tag);

    // 清空现有的星图标签数据
    await tagRepository.delete({ platform: '星图' });
    ...
    if (!fs.existsSync(csvFilePath)) {
      console.log('星图CSV文件不存在，跳过种子数据导入');
      return;
    }
    ...
    console.log(`读取到 ${csvData.length} 条星图标签数据`);
