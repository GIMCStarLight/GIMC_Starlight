...
export default class SeedHuahuoTags1758610867002 {
  public async run(dataSource: DataSource): Promise<any> {
    const tagRepository = dataSource.getRepository(Tag);

    // 清空现有的花火标签数据
    await tagRepository.delete({ platform: '花火' });
    ...
    if (!fs.existsSync(csvFilePath)) {
      console.log('花火CSV文件不存在，跳过种子数据导入');
      return;
    }
    ...
    console.log(`读取到 ${csvData.length} 条花火标签数据`);
  ...
  private async processTagHierarchy(
    ...
    for (let i = 0; i < levels.length; i++) {
      ...
      const newTag = tagRepository.create({
        ...
        description: `花火平台${i + 1}级标签：${levelValue}`,
      });
