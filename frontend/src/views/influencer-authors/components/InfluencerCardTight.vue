...
const handleSelectionChange = (data: any, selected: boolean) => {
  log.debug('[InfluencerCardTight] 选中状态变化:', {
  ...
  if (props.useStoreSelection !== false) {
    log.debug('[InfluencerCardTight] 更新store选中状态')
    // 直接设置选中状态，而不是切换
    store.setInfluencerSelectionSingle(data.author_id, selected)
    log.debug('[InfluencerCardTight] store当前选中数量:', store.selectedCount)
  } else {
    log.debug('[InfluencerCardTight] 跳过store更新（useStoreSelection=false）')
    // 如果不使用store，直接更新本地数据
