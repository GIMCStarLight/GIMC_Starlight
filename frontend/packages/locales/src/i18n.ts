...
async function loadLocaleMessages(lang: SupportedLanguagesType) {
  ...
  // Handle fallback locales - load zh-CN messages for 'zh' locale
  if (lang === 'zh-CN') {
    const zhCNMessage = await localesMap['zh-CN']?.();
    if (zhCNMessage?.default) {
      i18n.global.setLocaleMessage('zh', zhCNMessage.default);
    }
  }

  // Handle fallback locales - load en-US messages for 'en' locale
  if (lang === 'en-US') {
    const enUSMessage = await localesMap['en-US']?.();
    if (enUSMessage?.default) {
      i18n.global.setLocaleMessage('en', enUSMessage.default);
    }
  }
  ...
  // Also merge messages for fallback locales
  if (lang === 'zh-CN') {
    i18n.global.mergeLocaleMessage('zh', mergeMessage);
  }
  if (lang === 'en-US') {
    i18n.global.mergeLocaleMessage('en', mergeMessage);
  }
