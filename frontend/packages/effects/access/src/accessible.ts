...
async function generateAccessible(
  ...
  // 确保 accessCodes 是数组类型
  const accessCodes = Array.isArray(accessStore.accessCodes) ? accessStore.accessCodes : [];
  // 生成路由
  const accessibleRoutes = await generateRoutes(mode, options, accessCodes);
  ...
  switch (mode) {
    ...
    case 'mixed': {
      const [frontend_resultRoutes, backend_resultRoutes] = await Promise.all([
        generateRoutesByFrontend(routes, accessCodes, forbiddenComponent),
