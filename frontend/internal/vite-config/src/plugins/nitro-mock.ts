...
export const viteNitroMockPlugin = ({
  ...
  return {
    async configureServer(server) {
      ...
      // 如果默认端口被占用，使用可用端口并打印提示（而不是直接返回）
      const serverPort = availablePort;
      if (serverPort !== port) {
        consola.warn(
          `Port ${port} is occupied. Starting Nitro Mock Server on available port ${serverPort} instead.`,
        );
      }
      ...
      // 传入实际使用的端口
      runNitroServer(pkg.dir, serverPort, verbose);
      ...
      server.printUrls = () => {
        ...
        consola.log(
          `  ${colors.green('➜')}  ${colors.bold('Nitro Mock Server')}: ${colors.cyan(`http://localhost:${serverPort}/api`)}`,
        );
      };
    },
    enforce: 'pre',
